...
![图片不存在](./media/83ca0e031f642f5b7e13fbbf88ce4572.png)
...
![图片不存在](./media/2fef49c3d3665af0895d4710581132b5.png)
...
![图片不存在](./media/6b664410176830d72c98b440937acf7d.png)
...
![图片不存在](./media/437a4771e0f5abc3a073d3cb33ff804c.png)
...
![图片不存在](./media/c302a2f86df80bde44488c3416436ffd.png)

将数字引脚3的电平设置为低（即：PWM值为0），LED是熄灭。

![图片不存在](./media/28776c717d6696bf0b0b38fdf26bf506.png)

将数字引脚3的PWM值设置为i。

![图片不存在](./media/ef98a4c395e8089dacb384be2cf85146.png)

设置一个变量i，i从0逐渐增加到255，每一次都加1，总共加了255次， 每次以10毫秒的频率增加1，LED逐渐变亮。

![图片不存在](./media/c4b396c30e5bb810508b7d9a7e504b45.png)

设置一个变量i，i从255逐渐减少到0，每一次都减1，总共减了255次， 每次以10毫秒的频率减1，LED逐渐变暗。

![图片不存在](./media/df1d3f46038fc18fe1dc226138dba415.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\Mixly_代码**</u>”中，我们可以在此路径下打开代码文件"**07_Obstacle_Avoidance_Sensor**"。
...
在之前的项目中，我们研究了有源蜂鸣器，它只能发出一种声音，可能会让你觉得很单调。这个项目将学习另一种蜂鸣器，无源蜂鸣器。与有源蜂鸣器不同，无源蜂鸣器可以发出不同频率的声音。在这个项目中，我们将使用控制板控制无源蜂鸣器演奏一首歌曲。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\Mixly_代码**</u>”中，我们可以在此路径下打开代码文件''**24_TFT_Screen**"。
...
![图片不存在](media/cacb7acd817ef8d55435db75483a221c.png)

---

**4. 实验代码**

本项目中使用的代码保存在文件夹“<u>**...\项目代码\Mixly_代码**</u>”中，我们可以在此路径下打开代码文件''**25_Small_Table_Lamp**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/fef183dac061cec7e13fe46ab7ef9dc7.png)
...
![图片不存在](./media/95b9459e91c32a1361f36195068d8de6.png)

红色LED模块上的LED点亮。

![图片不存在](./media/e02c6ae05cff7b2ed85cdc924dbef9a4.png)
...
![图片不存在](media/bcd627cdd186ef247542a9b54f692a34.png)

---

**4. 实验代码**

本项目中使用的代码保存在文件夹“<u>**...\项目代码\Mixly_代码**</u>”中，我们可以在此路径下打开代码文件''**26_Automatic_Induction_Door**"。
...
![图片不存在](media/1580a1baaf98fb00bbed555b3d61fc71.png)

---

**4. 实验代码**

本项目中使用的代码保存在文件夹“<u>**...\项目代码\Mixly_代码**</u>”中，我们可以在此路径下打开代码文件''**27_PIR_Alarm**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/35a97ccd5483da8513653d981ee46bd8.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\Mixly_代码**</u>”中，我们可以在此路径下打开代码文件''**28_Smart_Window**"。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\Mixly_代码**</u>”中，我们可以在此路径下打开代码文件''**29_Night_Lamp**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

<span style="color: rgb(255, 76, 65);">注意：代码中的模拟值的阈值200可以根据实际情况设置。</span>

![图片不存在](./media/aea1bdb667add8388bd771eac89518c0.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\Mixly_代码**</u>”中，我们可以在此路径下打开代码文件''**30_Dimming_Lamp**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/9598d27fe21dd21c8a0928bb74d7e029.png)
...
![图片不存在](./media/625c6a0a8ceea00a96e6f1c96287a59b.png)

这是映射指令方块，可以将一个取值范围映射到另一个取值范围。

![图片不存在](./media/807ff8b1e28a21be8cd04d3499dd4502.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\Mixly_代码**</u>”中，我们可以在此路径下打开代码文件''**31_Temperature-humidity_meter**"。
...
![图片不存在](media/4619639de296759e56086df1b3145698.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\Mixly_代码**</u>”中，我们可以在此路径下打开代码文件''**32_TFT_displays_TF_card_pattern**"。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\Mixly_代码**</u>”中，我们可以在此路径下打开代码文件''**33_Intelligent_IR_Control**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/583d4fa5052d353fb40f16999faef995.png)
...
![图片不存在](./media/4bc65b8a6e5df76a7411835c29e86567.png)
...
按下红外遥控器上的 “ **②** ” 键，黄色LED点亮。串口监视器打印出按下的按键值和黄色LED的亮灯情况：“**<u>FF9867  yellow led on</u>**” 。

按下红外遥控器上的 “ **③** ” 键，红色LED和黄色LED都熄灭。串口监视器打印出按下的按键值和红色LED与黄色LED的亮灯情况：“**<u>FFB04F  two leds off</u>**” 。

![图片不存在](media/597366f86fc03521ec8ac8903dc1ad2c.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\Mixly_代码**</u>”中，我们可以在此路径下打开代码文件''**34_Corridor_Sensing_Light**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

<span style="color: rgb(255, 76, 65);">注意：代码中的模拟值的阈值可以根据实际情况设置。</span>

![图片不存在](./media/ea72104c6b697cf8e24135211e438127.png)
...
![图片不存在](./media/b45454158d42eb182ad9849ad963385c.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\Mixly_代码**</u>”中，我们可以在此路径下打开代码文件''**35_Control_Servo_and_LED**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

![图片不存在](./media/b4338976091f0d83b6801664fdae6fa6.png)
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\Mixly_代码**</u>”中，我们可以在此路径下打开代码文件''**36_Heat_abstractor**"。
...
本项目中使用的代码保存在文件夹“<u>**...\项目代码\Mixly_代码**</u>”中，我们可以在此路径下打开代码文件''**37_Ultrasonic_radar**"。

**注意：为了避免上传代码不成功，请上传代码前不要连接模块。代码上传成功后，拔下USB线断电，按照接线图正确接好模块后再用USB线连接到计算机上电，观察实验结果。**

<span style="color: rgb(255, 76, 65);">注意：代码中的距离阈值可以根据实际情况设置。</span>

![图片不存在](./media/d65a5134f836884f233cc52bdde28a74.png)
